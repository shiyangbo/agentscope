...
from agentscope.service.web.apiservice import api_request_for_big_model, api_request_for_dag_stream_api
...
class ASDiGraph(nx.DiGraph):
    ...
    def determine_current_node_running_status(self, node_id) -> (str, bool):
        ...
        elif self.validate_node_condition(node_id, predecessors_list):
            return WorkflowNodeStatus.RUNNING, True
        # 3. 节点前驱节点存在Switch节点, 且节点所在分支不满足条件, 置为未运行状态，节点返回空值
        else:
            # 3.1 节点前驱节点不存在Switch节点, 且节点前序节点存在未运行时，
    ...
    def validate_predecessor_condition(self, node_id, predecessors_list) -> bool:
        ...
        # 检查前驱节点是否有运行成功的节点，如果前驱节点全部为成功节点，则当前节点为”running“并且排除起始节点和分支器节点
        ...
        # 如果任意前驱节点处于 'running_skip' 状态，则不执行当前节点
    ...
    def update_conditions_pool_with_running_node(self, switch_node_id, target_node_id: str, condition_result: bool):
        ...
        # 但是由于GIL的机制天然保证了多线程之间的串行运行顺序，以及每个节点只操作字典params_pool的各自不同key，这里简单起见，可以不加锁
        # TODO 加 threading lock
        if switch_node_id not in self.conditions_pool:
            self.conditions_pool.setdefault(switch_node_id, {})[target_node_id] = condition_result
        else:
...
class PythonServiceUserTypingNode(WorkflowNode):
    ...
    def __call__(self, *args, **kwargs) -> dict:
        # 判断当前节点的运行状态
        self.running_status, is_running = self.dag_obj.determine_current_node_running_status(self.node_id)
        if not is_running:
            return {}
...
class RAGNode(WorkflowNode):
    ...
    def run(self, *args, **kwargs):
        ...
        if 'headers' not in params_dict['settings']:
            raise Exception("headers key not found in settings")
        # 2. 使用 api_request 函数进行 API 请求设置
        response = api_request_for_dag_stream_api(url=self.api_url, method='POST', headers=self.api_header,
                                              data=self.input_params_for_body)
        if response.status == service_status.ServiceExecStatus.ERROR:
            raise Exception(str(response.content))
        if response.content is None:
    ...
    def convert_rag_request(origin_params_dict: dict) -> dict:
        ...
        for item in origin_params_dict['inputs']:
            if item['name'] == 'input':
                item['name'] = 'question'
        # 添加知识库url地址
        origin_params_dict['settings']['url'] = 'https://maas-gz-api.ai-yuanjing.com/openapi/knowledge/stream/search'
        return origin_params_dict
